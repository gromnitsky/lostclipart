...
export function my_fetch(url, opt) {
    let fetcherr = r => {
        if (r.ok) return r
        let e = new Error(r.status + ' ' + r.statusText)
        e.headers = r.headers
        throw e
    }
...
export function Tags(props) {
    let tags = (props.csv || '')
        .split(',').map(v => v.trim()).map( (name, idx) => {
            let p = encodeURIComponent('-t '+ search.sq(name))
            return <Link key={idx} to={`/search/${p}`}>{name}</Link>
        })
    return <span>{tags.reduce((prev, curr) => [prev, ', ', curr])}</span>
}

export function gui_error(err, state_var = 'error') {
